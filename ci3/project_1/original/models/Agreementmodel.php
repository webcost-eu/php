<?php
class Agreementmodel extends MainModel {

	private $data=array();
	public function __construct()
	{
		parent::__construct();
		$this->table = TABLE_AGREEMENTS;
	}
	...
	public function getAgreementDetails($id)
	{
		$this->db->select('c.*, d.name as deal_name, u.firstName as cons_fname, u.lastName as cons_lname, a.firstName as agent_fname, a.lastName as agent_lname, s.translation_index, l.lastName as layer_lname, l.firstName as layer_fname ')->from($this->table.' c');
		$this->db->join(TABLE_USER.' u', 'c.consultant_id = u.id');
		$this->db->join(TABLE_USER.' a', 'c.agent_id = a.id');
		$this->db->join(TABLE_USER.' l', 'c.solicitor_id = l.id');
		$this->db->join(TABLE_CLIENT_SELECT_OPTION.' s', 'c.status_id = s.id');
